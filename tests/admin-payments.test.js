...
const { default: PaymentOrder } =
  await import("../src/modules/payments/payment.model.js");
...
describe("admin top-up management", () => {
  ...
  beforeEach(async () => {
    ...
    notificationMocks.sendTopUpFailedNotification
      .mockReset()
      .mockResolvedValue(null);
    notificationMocks.sendAdminWalletAdjustmentNotification
      .mockReset()
      .mockResolvedValue(null);
    const admin = await User.create({
      fullName: "Payment Admin",
      email: "payment-admin@werent.vn",
  ...
  it("applies promotion, credits a webhook only once, and exposes transaction details", async () => {
    ...
    const promotion = await request(app)
      .post("/api/admin/payments/promotions")
  ...
  it("marks a top-up as failed via webhook and notifies only once", async () => {
    ...
    expect(notificationMocks.sendTopUpFailedNotification).toHaveBeenCalledWith(
      expect.objectContaining({
        email: "topup@example.com",
      }),
      expect.objectContaining({
  ...
  it("keeps package payments separate and does not credit the wallet", async () => {
    ...
    const topupScope = await request(app)
      .get("/api/admin/payments/transactions")
      .set("Authorization", `Bearer ${adminToken}`)
      .query({ scope: "topup" });
  ...
  it("creates an auditable admin adjustment and prevents negative balances", async () => {
    const credit = await request(app)
      ...
      .send({
        ...
        direction: "credit",
        amount: 200000,
        reason: "Bồi hoàn giao dịch lỗi.",
      });
    expect(credit.status).toBe(201);
    expect(credit.body.data.item).toMatchObject({
      transactionType: "admin_adjustment",
      totalCredit: 200000,
      balanceBefore: 0,
      balanceAfter: 200000,
      adjustmentReason: "Bồi hoàn giao dịch lỗi.",
    });
    expect(
      notificationMocks.sendAdminWalletAdjustmentNotification,
    ).toHaveBeenCalledOnce();
    ...
    const invalidDebit = await request(app)
      .post("/api/admin/payments/adjustments")
      .set("Authorization", `Bearer ${adminToken}`)
      .send({
        userId: user.id,
        direction: "debit",
        amount: 300000,
        reason: "Thu hồi tiền ghi nhận sai.",
      });
  ...
  it("lists current wallet top-up promotions for the signed-in user", async () => {
    ...
    await request(app)
      ...
      .send({
        ...
        minimumAmount: 10000,
        startsAt: new Date(now + 60000),
        endsAt: new Date(now + 120000),
      });

    const response = await request(app)
      .get("/api/payments/top-up/promotions")
      .set("Authorization", `Bearer ${userToken}`);
  ...
  it("credits promotion money for QR checkout payments confirmed by SePay IPN", async () => {
    ...
    expect(notificationMocks.sendTopUpSuccessNotification).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        orderCode: checkout.body.data.order.orderCode,
        amount: 200000,
        bonusAmount: 30000,
        totalCredit: 230000,
        balanceAfter: 230000,
  ...
  it("marks a QR top-up as failed via SePay IPN and notifies only once", async () => {
    const checkout = await request(app)
      .post("/api/payments/top-up/checkout")
      .set("Authorization", `Bearer ${userToken}`)
      .send({ amount: 200000, paymentMethod: "qr" });
    ...
    const failedPayload = {
      ...
      order: {
        id: "SEPAY-ORDER-FAILED-1",
        order_invoice_number: checkout.body.data.order.orderCode,
        order_amount: 200000,
        order_status: "FAILED",
        order_description: "QR checkout failed payment",
      },
      transaction: {
        transaction_id: "SEPAY-QR-IPN-FAILED-1",
        transaction_amount: 200000,
    ...
    expect(refreshedUser.walletPromotionBalance).toBe(0);
    const history = await WalletTransaction.find({
      paymentOrder: refreshedOrder._id,
    });
  ...
  it("chooses the highest priority promotion before comparing bonus amounts", async () => {
    ...
    await request(app)
      ...
      .set("Authorization", `Bearer ${adminToken}`)
      .send({
        name: "Better money lower priority",
        code: "LOWBEST",
        bonusPercent: 50,
        minimumAmount: 10000,
        maximumBonus: 50000,
  ...
  it("uses the selected top-up promotion when checkout receives a promotion id", async () => {
    ...
    const checkout = await request(app)
      ...
      .send({
        amount: 100000,
        paymentMethod: "qr",
        promotionIds: [selected.body.data.item._id],
      });
  ...
  it("stacks selected top-up promotions only when every selected campaign is stackable", async () => {
    ...
    const checkout = await request(app)
      ...
      .send({
        amount: 100000,
        paymentMethod: "qr",
        promotionIds: [first.body.data.item._id, second.body.data.item._id],
      });
    ...
    expect(
      checkout.body.data.order.promotions.map((promotion) => promotion.code),
    ).toEqual(["STACK10", "STACK15"]);
    ...
    const blocked = await request(app)
      ...
      .send({
        amount: 100000,
        paymentMethod: "qr",
        promotionIds: [first.body.data.item._id, exclusive.body.data.item._id],
      });
  ...
  it("auto-applies eligible top-up promotions without exposing them as selectable campaigns", async () => {
    ...
    expect(promotions.body.data.items.map((item) => item.code)).toEqual([
      "MANUAL10",
    ]);
    expect(promotions.body.data.autoItems.map((item) => item.code)).toEqual([
      "AUTO2M",
    ]);
  ...
  it("keeps a QR promotion snapshot when the promotion is edited before payment", async () => {
    ...
    const update = await request(app)
      .patch(`/api/admin/payments/promotions/${promotion.body.data.item._id}`)
      .set("Authorization", `Bearer ${adminToken}`)
  ...
  it("reserves promotion usage only while pending QR top-up orders are valid", async () => {
    ...
    const reservedOrder = await request(app)
      .post("/api/payments/topups")
      .set("Authorization", `Bearer ${userToken}`)
      .send({ amount: 100000, provider: "sepay" });
  ...
  it("creates a demo top-up, separates real and promotion balances, and records wallet history", async () => {
    ...
    const quote = await request(app)
      .get("/api/admin/payments/demo-topups/quote")
      .set("Authorization", `Bearer ${adminToken}`)
    ...
    const response = await request(app)
      .post("/api/admin/payments/demo-topups")
      .set("Authorization", `Bearer ${adminToken}`)
      .send({
        email: user.email,
        amount: 500000,
        note: "Demo nạp tiền Sepay.",
      });
