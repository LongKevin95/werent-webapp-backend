...
async function cleanupDocuments(documents) {
  await Promise.all(
    documents.map((document) => deleteAsset(document.publicId, "image").catch(() => null)),
  );
}
...
export async function submitAccountKyc(userId, payload, files = {}) {
  ...
  const pending = await KYCRequest.exists({ user: userId, status: KYC_STATUS.PENDING });
  ...
  if (requiredFiles.some((entry) => !entry.file)) {
    throw new ApiError(400, "Vui lòng tải đủ CCCD mặt trước, mặt sau và ảnh selfie.");
  }
  if (requiredFiles.some((entry) => !entry.file.mimetype?.startsWith("image/"))) {
...
export function getLatestAccountKyc(userId) {
  return KYCRequest.findOne({ user: userId }).sort({ createdAt: -1 })
    .populate("reviewedBy", "fullName email");
}

export async function submitListingVerification(user, propertyId, payload, files = []) {
  assertObjectId(propertyId, "Không tìm thấy tin đăng.");
  if (user.kycStatus !== KYC_STATUS.VERIFIED || !user.canPostListing) {
    throw new ApiError(403, "Bạn cần xác thực tài khoản trước khi xác thực bất động sản.");
  }
  ...
  if (!files.length) throw new ApiError(400, "Vui lòng tải ít nhất một giấy tờ chứng minh.");

  const pending = await ListingVerificationRequest.exists({
    property: propertyId,
    status: LISTING_VERIFICATION_STATUS.PENDING,
  });
  if (pending) throw new ApiError(409, "Tin đăng đang có hồ sơ xác thực chờ duyệt.");
...
export function getLatestListingVerification(userId, propertyId) {
  return ListingVerificationRequest.findOne({ user: userId, property: propertyId })
    .sort({ createdAt: -1 })
    .populate("reviewedBy", "fullName email");
}
...
export async function listAccountKycRequests(query = {}) {
  ...
  const [items, total] = await Promise.all([
    KYCRequest.find(filter).populate("user", "fullName email phone avatarUrl kycStatus canPostListing")
      .populate("reviewedBy", "fullName email").sort({ createdAt: -1 })
...
export async function getAccountKycRequest(requestId) {
  ...
  const item = await KYCRequest.findById(requestId)
    .populate("user", "fullName email phone avatarUrl kycStatus canPostListing verifiedAt")
    .populate("reviewedBy", "fullName email");
...
export async function reviewAccountKyc(requestId, adminId, payload) {
  ...
  await Notification.create({
    ...
    title: payload.status === KYC_STATUS.VERIFIED ? "Tài khoản đã được xác thực" : "Cập nhật hồ sơ xác thực",
    message: payload.status === KYC_STATUS.VERIFIED
...
export async function listListingVerificationRequests(query = {}) {
  ...
  const [items, total] = await Promise.all([
    ListingVerificationRequest.find(filter)
      ...
      .populate("property", "title propertyType address area bedrooms bathrooms projectName status verificationStatus")
      .populate("reviewedBy", "fullName email").sort({ createdAt: -1 })
      .skip((page - 1) * limit).limit(limit),
...
export async function getListingVerificationRequest(requestId) {
  ...
  const item = await ListingVerificationRequest.findById(requestId)
    .populate("user", "fullName email phone identityNumber address dateOfBirth kycStatus verifiedAt")
    .populate("property")
    .populate("reviewedBy", "fullName email");
...
export async function reviewListingVerification(requestId, adminId, payload) {
  ...
  property.verifiedAt = [
    LISTING_VERIFICATION_STATUS.VERIFIED_OWNER,
    LISTING_VERIFICATION_STATUS.VERIFIED_AUTHORIZED,
  ].includes(payload.status) ? new Date() : null;
  ...
  await Notification.create({
    ...
    title: property.verifiedAt ? "Bất động sản đã được xác thực" : "Cập nhật xác thực bất động sản",
    message: property.verifiedAt ? "Tin đăng đã nhận badge Xác thực." : payload.reason,
    metadata: { requestId: item._id, propertyId: property._id, status: payload.status },
  });
  return item;
}
