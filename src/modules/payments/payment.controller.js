...
export const getPackages = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ success: true, message: "Lấy danh sách gói thanh toán thành công.", data: { items } });
});

export const createPaymentOrder = asyncHandler(async (req, res) => {
  const order = await createOrder(req.user._id, req.body);
  return res.status(201).json({ success: true, message: "Tạo đơn thanh toán thành công.", data: { order } });
});

export const createTopUpCheckout = asyncHandler(async (req, res) => {
  const data = await createWalletTopUpCheckout(req.user, req.body, { origin: req.get("origin") });
  return res.status(201).json({ success: true, message: "Tạo yêu cầu nạp tiền thành công.", data });
});

export const createTopupOrder = asyncHandler(async (req, res) => {
  const order = await createWalletTopupOrder(req.user._id, req.body);
  return res.status(201).json({ success: true, message: "Tạo lệnh nạp tiền thành công.", data: { order } });
});

export const getMyPaymentHistory = asyncHandler(async (req, res) => {
  const items = await getPaymentHistory(req.user._id);
  return res.status(200).json({ success: true, message: "Lấy lịch sử thanh toán thành công.", data: { items } });
});
...
export const getPaymentQr = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ success: true, message: "Tạo thông tin QR thanh toán thành công.", data });
});

export const sepayWebhook = asyncHandler(async (req, res) => {
  const signature = req.headers["x-sepay-signature"] ?? "";
  const order = await handleSepayWebhook(req.body, String(signature), req.rawBody ?? req.body);
  return res.status(200).json({ success: true, message: "Xử lý webhook SePay thành công.", data: { order } });
});
